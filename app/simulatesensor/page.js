            ...
            <div className="text-white mt-24 mb-48">
                <form className=" text-white ml-16 pr-20 ">
                    <div className="mb-3 w-1/2">
                        <label for="email" className="block mb-2 text-lg font-medium text-white">Longitude</label>
                        <input type="number" 
                        value={longitude} 
                        onChange={handleLongitudeChange} 
                        step="0.000001"  className="bg-gray-50 border border-gray-300 text-gray-900 font-medium text-sm             rounded-lg block w-full p-1.5" placeholder="Longitude" />
                    </div>

                    <div className="mb-5 w-1/2">
                    <label for="email" className="block mb-2 text-lg font-medium text-white">Latitude</label>
                    <input type="number" 
                        value={latitude} 
                    ...
                    <div className="mb-3 w-1/2">
                    <label for="email" className="block mb-2 text-lg font-medium text-white">Voltage (V)</label>
                        <input type="number" 
                        value={voltage} 
                        onChange={handleVoltageChange} 
                        step="0.1"  className="bg-gray-50 border border-gray-300 text-gray-900 font-medium text-sm rounded-lg block w-full p-1.5" placeholder="voltage" />
                    </div>

                    <div className="mb-5 w-1/2">
                    <label for="email" className="block mb-2 text-lg font-medium text-white">Current (A)</label>
                        <input type="number" 
                        value={current} 
                        onChange={handleCurrentChange} 
                        step="0.1"   className="bg-gray-50 border border-gray-300 text-gray-900 font-medium text-sm rounded-lg block w-full p-1.5" placeholder="current" />
                    </div>
            </form>        
            <br />
            {!capturing ? (
                <button className="ml-16 text-white bg-green-600 hover:bg-green-700  font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-1.5 text-center " type="button" onClick={startCapture}>Start Capture</button>
            ) : (
                <button className="ml-16 text-white bg-red-500 hover:bg-red-700  font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-1.5 text-center " type="button" onClick={stopCapture}>Stop Capture</button>
            )}
